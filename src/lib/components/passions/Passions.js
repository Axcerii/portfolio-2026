...
export default function Passions() {
    ...
    return (
        <section id="passions" className="relative h-screen min-h-screen w-full bg-foreground flex flex-col md:flex-row overflow-x-hidden">
            ...
            <div
                className={`hidden md:block absolute top-1/5 left-1/2 -translate-y-1/2 z-30 pointer-events-none transition-all duration-1000 ease-[cubic-bezier(0.25,1,0.25,1)] w-80
            ...
            <div className="md:hidden pt-20 pb-4 px-6 z-20 flex flex-col items-center">
            ...
            <div className="relative w-full flex-1 flex flex-row overflow-x-auto snap-x snap-mandatory md:overflow-x-visible md:snap-none z-10 no-scrollbar">
                ...
                >
                    ...
                    <div className={`hidden md:block absolute inset-0 bg-linear-to-br from-secondary/30 to-transparent transition-opacity duration-1000 ease-in-out ${hovered === 1 ? 'opacity-100' : 'opacity-0'} z-10`} />

                    {/* Background Image for Panel 1 */}
                    <div
                        className="absolute inset-0 bg-cover bg-center opacity-[0.05] sm:opacity-[0.03] group-hover:opacity-10 md:group-hover:opacity-15 transition-all duration-1000 ease-in-out mix-blend-luminosity z-0 group-hover:scale-105"
                        style={{ backgroundImage: `url(${presentationContent.passions[0].bgImage})` }}
                    />

                    <div className="relative z-20 flex flex-col items-center max-w-lg cursor-default w-full">
                        {/* Image Icon */}
                ...
                <div
                    className="hidden md:block w-48 lg:w-[12vw] shrink-0 h-full cursor-default z-20"
                ...
                >
                    ...
                    <div className={`hidden md:block absolute inset-0 bg-linear-to-bl from-accent/20 to-transparent transition-opacity duration-1000 ease-in-out ${hovered === 2 ? 'opacity-100' : 'opacity-0'} z-10`} />

                    {/* Background Image for Panel 2 */}
                    <div
                        className="absolute inset-0 bg-cover bg-center opacity-[0.05] sm:opacity-[0.03] group-hover:opacity-10 md:group-hover:opacity-15 transition-all duration-1000 ease-in-out mix-blend-luminosity z-0 group-hover:scale-105"
                        style={{ backgroundImage: `url(${presentationContent.passions[1].bgImage})` }}
                    />
