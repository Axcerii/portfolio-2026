...
export default function BottomNav() {
    ...
    return (
        <div
            className={`fixed bottom-4 md:bottom-8 left-0 w-full z-2000 transition-all duration-700 pointer-events-none flex justify-end md:justify-center px-6 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
                }`}
        >
            {/* Desktop Navigation */}
            <nav className="hidden md:flex flex-wrap justify-center gap-x-6 gap-y-2 bg-foreground/60 backdrop-blur-md px-8 py-3 rounded-full border border-accent/50 shadow-2xl pointer-events-auto max-w-[90vw]">
                {links.map((link, index) => (
                    <a
                        key={index}
                        href={link.href}
                        onClick={(e) => handleClick(e, link.href)}
                        className="group relative uppercase text-background/80 hover:text-accent transition-colors duration-300 text-sm lg:text-base py-1 px-2 underline-center"
                    >
